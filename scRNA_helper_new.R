...
# # add expression of gene in conos plat as new level in anno
# add_gene_to_anno <- function(gene_name) { # to add a new level to anno
#   levels(test) <- c(levels(test),gene_name)
#   x <- con_org$plotGraph(gene=gene_name)  
#   x <- x$data[x$data$Color>0,]$CellName
#   
#   # to add a new level to anno
#   levels(test) <- c(levels(test),gene_name)
#   
#   test[x] <- gene_name # must be a factor level that already exists
#   
#   return(test) 
# }

#use: test <- add_gene_to_anno()
...
#     }
#     frame2
#     saveRDS(frame2, file = "x.rds") #remember to change this dataframe name once you do a new cell selection
#   })
#   
# }
# 
...
# ```{r}
# add_selection_to_anno <- function(x, name) { #name should be in " " it is the nam of the new level, x is the name of the data frame selected in shiney app
...
#use: anno <- add_selection_to_anno()
# ```
